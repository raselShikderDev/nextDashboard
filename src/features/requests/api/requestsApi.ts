...
export const requestsApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getRequests: builder.query<PaginatedResponse<Request>, FilterParams>({
      query: (params = {}) => {
        const qs = new URLSearchParams();
        Object.entries(params).forEach(([k, v]) => { if (v !== undefined && v !== "") qs.set(k, String(v)); });
        return `/requests?${qs.toString()}`;
      },
      transformResponse: (res: { data: PaginatedResponse<Request> }) => res.data,
      providesTags: (result) => result ? [...result.data.map(({ id }) => ({ type: "Request" as const, id })), { type: "Request", id: "LIST" }] : [{ type: "Request", id: "LIST" }],
    }),
    ...
    updateRequest: builder.mutation<Request, { id: string; body: Partial<Request> }>({
      query: ({ id, body }) => ({ url: `/requests/${id}`, method: "PATCH", body }),
      transformResponse: (res: { data: Request }) => res.data,
      invalidatesTags: (_r, _e, { id }) => [{ type: "Request", id }, { type: "Request", id: "LIST" }],
    }),
    ...
    rejectRequest: builder.mutation<Request, { id: string; notes?: string }>({
      query: ({ id, notes }) => ({ url: `/requests/${id}/reject`, method: "PATCH", body: { notes } }),
      transformResponse: (res: { data: Request }) => res.data,
      invalidatesTags: (_r, _e, { id }) => [{ type: "Request", id }],
    }),
  }),
...
export const { useGetRequestsQuery, useGetRequestByIdQuery, useCreateRequestMutation, useUpdateRequestMutation, useDeleteRequestMutation, useApproveRequestMutation, useRejectRequestMutation } = requestsApi;
